//You may turn off this auto-detection by giving explicit names
...
function calculate(expression) {
	try {
		if (regexp.eval.test(expression)) {
...
function equal(a, b) {
	...
	if (a.toString && a.toString() != b.toString()) {
		return false;
	}
...
function listToArray(str) {
	if (!isEmpty(str)) {
		return $.map(str.toString().split(','), function(item) {
...
function addUserToCache(user) {
	if ($.inArray(user, cache.users) < 0) {
		cache.users.push(user);
		cache.users.sort(caseInsensitiveSortOrder);
	}
}
...
function addTagsToCache(tags) {
	$.each(tags, function(i, tag) {
		if ($.inArray(tag, cache.tags) < 0) {
			cache.tags.push(tag);
		}
	});
	cache.tags.sort(caseInsensitiveSortOrder);
}
...
function refreshLongCache(callback) {
...
function refreshShortCache(callback) {
	db.view(design + '/cache', {
		success: function(data) {
			cache.boards = {};
			cache.components = {};
			cache.types = {};
			...
			$.each(data.rows, function(i, doc) {
				switch (doc.key) {
					case 'board':
						cache.boards[doc.value._id] = doc.value;
						break;

					case 'component':
						cache.components[doc.value._id] = doc.value;
						break;

					case 'type':
						cache.types[doc.value._id] = doc.value;
						break;

					case 'version':
						cache.versions[doc.value._id] = doc.value;
						break;
				}
			});
...
function editableSelectChange() {
	...
	if (last.length == 0 || last.attr('selected')) {
		var focus = sel.is(':focus');
		var input = $('<input type="text" style="display: inline"/>');
		sel.after(input);
		sel.remove();
		input.attr('id', sel.attr('id'));
...
function ui(elem, optTr) {
	var elem = $(elem || '#main');
	var tr = optTr || elem.tr();
	...
	elem.find('select.editable')
		.append($('<option/>').text(tr('...')))
		.change(editableSelectChange)
		.filter(':not(:hidden)')
		.change();
}
...
$(function() {
	...
	$.couch.app(function(app) {
		...
		reloadPage = function() {
			if (page) {
				$('#main').trigger(page.event, page.args);
			}
		};
